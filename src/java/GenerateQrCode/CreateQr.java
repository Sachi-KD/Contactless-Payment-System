...
public class CreateQr {
    ...
    public void createQr() {
        ...
        String filePath = "C:\\Users\\sachi\\Documents\\NetBeansProjects\\CPS\\web\\assets\\QrImage\\JD.png";
        int size = 1125;
        String fileType = "png";
    ...
    public boolean SaveQrCode(int UserID, int ticketId) {
        ...
        String imagePath = "C:\\Users\\sachi\\Documents\\NetBeansProjects\\CPS\\web\\assets\\QrImage\\JD.png";
        try {
            conn.connectMysql();
            state = conn.con.createStatement();
            ResultSet res = state.executeQuery("SELECT MAX(`TicketID`) FROM ticket");
            int LastIntId = 0;
            while (res.next()) {
            ...
            if (isRealesed(UserID, ticketId)) {
                ...
                prepair = conn.con.prepareStatement("INSERT INTO ticket VALUES(?,?,?,?,?,?);");
                if (LastIntId == 0) {
                    prepair.setInt(1, 1);
                    LastIntId = 1;
                } else {
    ...
    public boolean isRealesed(int userId, int ticketId) {
        ...
        try {
            conn.connectMysql();
            state = conn.con.createStatement();
            ResultSet res = state.executeQuery("SELECT * FROM `user`.ticket WHERE `FunID` = " + ticketId + " AND `UserID` = " + userId + "");
    ...
    public void imageDataGet(int UserID, int FunId) {
        String outputPath = "C:\\Users\\sachi\\Documents\\NetBeansProjects\\CPS\\web\\assets\\QrImage\\JD.png";
        //byte[] imageData = null;
        try {
            // Assume 'connection' is your database connection
            conn.connectMysql();
            
            PreparedStatement stmt = conn.con.prepareStatement("SELECT QrImage FROM `user`.ticket WHERE `UserID` = " + UserID + " AND `FunID` = " + FunId + "");
